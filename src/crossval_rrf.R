...
for (cValidationRun in 1:10){
  ...
  for (cModel in 1:n.models){
    ...
    mod.rrf <- RRF(X.train,as.factor(y),importance=T,coefReg=0.3)
    rrf.preds <- predict(mod.rrf,X.test,type="prob")
    preds[binindex.test,1] <- rrf.preds[,2]
    
    mod.rrf <- RRF(X.train,as.factor(y),importance=T,coefReg=0.2)
    rrf.preds <- predict(mod.rrf,X.test,type="prob")
    preds[binindex.test,2] <- rrf.preds[,2]
    
    mod.rrf <- RRF(X.train,as.factor(y),importance=T,coefReg=0.1)
    rrf.preds <- predict(mod.rrf,X.test,type="prob")
    preds[binindex.test,3] <- rrf.preds[,2]

    mod.rrf <- RRF(X.train,as.factor(y),importance=T,coefReg=0.07)
    rrf.preds <- predict(mod.rrf,X.test,type="prob")
    preds[binindex.test,4] <- rrf.preds[,2]
    
    mod.rrf <- RRF(X.train,as.factor(y),importance=T,coefReg=0.05)
    rrf.preds <- predict(mod.rrf,X.test,type="prob")
    preds[binindex.test,5] <- rrf.preds[,2]
