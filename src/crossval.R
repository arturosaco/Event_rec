...
for (cValidationRun in 1:nValidation){
...
n.models <- nrow(subs$resList$test.subset)
...
for (cModel in 1:n.models){
  ...
  ##########################################################
    ...
    mod.glm <- glmnet(x = X.train, y = y, family = "binomial", lambda = 2)
    glmnet.preds <- predict(mod.glm, newx = as.matrix(X.test), type = "response") 
    preds[binindex.test,1] <- glmnet.preds  
    
  mod.glm <- glmnet(x = X.train, y = y, family = "binomial", lambda = 0.05)
  glmnet.preds <- predict(mod.glm, newx = as.matrix(X.test), type = "response") 
  preds[binindex.test,2] <- glmnet.preds
  
  mod.glm <- glmnet(x = X.train, y = y, family = "binomial", lambda = 0.03)
  glmnet.preds <- predict(mod.glm, newx = as.matrix(X.test), type = "response") 
  preds[binindex.test,3] <- glmnet.preds
  
  mod.glm <- glmnet(x = X.train, y = y, family = "binomial", lambda = 0.01)
  glmnet.preds <- predict(mod.glm, newx = as.matrix(X.test), type = "response") 
  preds[binindex.test,4] <- glmnet.preds
  
  mod.glm <- glmnet(x = X.train, y = y, family = "binomial", lambda = 0.008)
  glmnet.preds <- predict(mod.glm, newx = as.matrix(X.test), type = "response") 
  preds[binindex.test,5] <- glmnet.preds
...
avg.mapk.lasso <- colMeans(mapk.lasso)
plot(avg.mapk.lasso)